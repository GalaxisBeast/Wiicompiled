        ...
        for (int physicalPort = 0; physicalPort < PAD_CHANMAX; ++physicalPort) {
            ...
            if (ImGui::MenuItem(label.c_str(), nullptr, adapterAssignment == physicalPort)) {
                for (uint32_t gamePort = 0; gamePort < PAD_CHANMAX; ++gamePort) {
                    if (gamePort != selectedGamePort &&
                        Wup028Adapter::GetPortAssignment(gamePort) == physicalPort) {
...
void InitializeRuntimeSettings() noexcept {
    ...
    // VR presents the game's native 60 Hz frames. Desktop interpolation multiplies GPU work and
    // blocks each synthetic frame on the headset compositor.
    ...
    const uint32_t targetFps =
        ...
        0;
#else
        kFrameInterpolationTargetFps[static_cast<size_t>(g_frameInterpolationMode)];
#endif
    LimitResolutionForFrameRate();
    aurora_set_frame_interpolation_fps(targetFps);
